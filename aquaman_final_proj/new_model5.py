...
async def get_sensor_data(farmer_data: FarmerData = Depends(get_farmer_data)):
    """Get current sensor data."""
    try:
        if not data_store.sensor_data:
            return None
        
        latest_data = data_store.sensor_data[-1]
        return SensorData(
        ...
        raise HTTPException(
            ...
            detail="Error retrieving sensor data"
        )

# Endpoint to post new sensor data
@app.post("/sensor", response_model=SensorData)
async def post_sensor_data(sensor_data: SensorData, farmer_data: FarmerData = Depends(get_farmer_data)):
    """Post new sensor data."""
    try:
        # Add the new data to our store
        data_store.sensor_data.append({
        ...
        # Get current day since culture start
        days = (sensor_data.timestamp - farmer_data.culture_start_date).days
        
        # Calculate shrimp weight and growth rate
        lunar_phase = get_lunar_phase(sensor_data.timestamp)
        shrimp_weight = calculate_growth(days, df.iloc[-1], lunar_phase)
        growth_rate = (shrimp_weight - df.iloc[-2]["shrimp_weight"] if len(df) > 1 else shrimp_weight) / days
        
        # Calculate FCR and feed schedule
        fcr, feed_schedule = calculate_fcr_and_feed_schedule(
            df, 
            farmer_data.stocking_density, 
            farmer_data.pond_area
        )
        
        # Predict yield
        predicted_yield_kg = predict_yield(
            df, 
            farmer_data.stocking_density, 
            farmer_data.pond_area
        )
        ...
        analysis = AnalysisResponse(
            water_quality=WaterQualityParameters(
                pH=df.iloc[-1]["pH"],
                temperature=df.iloc[-1]["temperature"],
                turbidity=df.iloc[-1]["turbidity"],
                tds=df.iloc[-1]["tds"],
                salinity=df.iloc[-1]["salinity"],
                ammonia=df.iloc[-1]["ammonia"],
                do=df.iloc[-1]["do"],
                nitrite=df.iloc[-1]["nitrite"],
                alkalinity=df.iloc[-1]["alkalinity"]
            ),
        ...
        return CustomJSONResponse(content=analysis)
    except Exception as e:
...
# Endpoint to capture simulated sensor data
...
# Endpoint to get current sensor data
@app.get("/get_sensor_data", response_model=SensorData)
async def get_sensor_data(farmer_data: FarmerData = Depends(get_farmer_data)):
    """Get current sensor data."""
    try:
        if not data_store.sensor_data:
...
@app.get("/analyze", response_class=CustomJSONResponse)
async def analyze_data(farmer_data: FarmerData = Depends(get_farmer_data)):
    ...
    try:
        ...
        water_quality_data = {
            ...
            "turbidity": round(df["turbidity"].mean(), 2),
            "tds": round(df["tds"].mean(), 2),
            "salinity": round(df["salinity"].mean(), 2),
            "ammonia": round(df["ammonia"].mean(), 2),
            "do": round(df["do"].mean(), 2),
            "nitrite": round(df["nitrite"].mean(), 2),
            "alkalinity": round(df["alkalinity"].mean(), 2)
        }
